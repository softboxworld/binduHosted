...
create table public.services (
  ...
  constraint unique_name_cost unique (name, cost),
...
create index IF not exists projects_organization_id_idx on public.worker_projects using btree (organization_id) TABLESPACE pg_default;

create index IF not exists idx_projects_organization on public.worker_projects using btree (organization_id) TABLESPACE pg_default;
...
create table public.tasks (
  ...
  constraint tasks_project_id_fkey foreign KEY (project_id) references worker_projects (id) on delete CASCADE,
...
create index IF not exists idx_tasks_date on public.tasks using btree (due_date) TABLESPACE pg_default;
...
create table public.orders (
  ...
  payment_status text not null default 'partially_paid'::text,
...
create table public.order_workers (
  ...
  constraint order_workers_project_id_fkey foreign KEY (project_id) references worker_projects (id) on delete RESTRICT,
...
after INSERT on order_workers for EACH row
...
create table public.categories (
  ...
  constraint categories_organization_id_fkey foreign KEY (organization_id) references organizations (id)
) TABLESPACE pg_default;
...
create index IF not exists idx_reference on public.payments using btree (reference_id, reference_type) TABLESPACE pg_default;

create index IF not exists idx_organization on public.payments using btree (organization_id) TABLESPACE pg_default;

create index IF not exists idx_created_at on public.payments using btree (created_at) TABLESPACE pg_default;
